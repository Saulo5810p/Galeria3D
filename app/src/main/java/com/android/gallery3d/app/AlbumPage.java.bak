...
public class AlbumPage extends ActivityState implements GalleryActionBar.ClusterRunner,
    ...
    // Passo 6: mMediaSet aqui e diretamente o ClusterAlbum aberto (a pasta
    // de faixas). Filtra in-memory; notifyContentChanged() dentro de
    // setSearchFilter() ja avisa o AlbumDataLoader (mAlbumDataAdapter), que
    // escuta o ContentListener do MediaSet e recarrega o SlotView sozinho.
    @Override
    public void onSearchQueryChanged(String query) {
        if (mMediaSet instanceof ClusterAlbum) {
            ((ClusterAlbum) mMediaSet).setSearchFilter(query);
        }
    }
